...
DAG 2 is initiated using the trigger process from DAG 1. This DAG comprises two key processes: training data and concluding the DAG. The training process is implemented in the train.py file, which includes loading .pkl files generated by DAG 1, addressing class imbalance, and performing hyperparameter tuning to optimize the model's performance.
The train.py, Dockerfile, and requirements.txt files are stored on the GCP Artifact Registry. The Dockerfile contains instructions for running the train.py file, while the requirements.txt lists all necessary dependencies. Vertex AI is utilized to execute the train.py file stored on GCP, using the Dockerfile to guide the process. After training and evaluating the models, the best-performing model is selected and saved as saved_model.pkl on GCP for future use.

CALLING .PKL FILES - From DAG 1, X_train, X_test, Y_train and Y_test .pkl files are called to DAG 2 for training and pre-processing the data.

CLASS IMBALANCE - The train.py script is designed to address the significant class imbalance in our dataset, which contains a large number of records for patients without sepsis and a relatively small number of records for patients with sepsis. To ensure accurate and fair model training, train.py employs techniques to balance both classifications, thereby improving the model's ability to predict sepsis accurately despite the initial imbalance in the dataset.

HYPERPARAMATER TUNING -  Hyperparameter tuning is performed using GridSearchCV to optimize our model's performance. This tuning process not only enhances accuracy but also reduces the number of false positives, leading to more precise classifications. We compared the hyperparameters of several models, including Random Forest, XGBoost Classifier, and Logistic Regression. Among these, the XGBoost Classifier emerged as the best performing model, achieving an impressive F1 score of 0.9.
